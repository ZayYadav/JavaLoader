...
/** Hybrid RSA-OAEP + AES-256-GCM request/response envelope. */
@Obfuscate
final class ParallaxKaRaazHai {
    ...
    private ParallaxKaRaazHai() {
    }

    static RequestEnvelope encryptRequest(JSONObject payload, String publicKeyBase64) throws Exception {
        ...
        aes.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(sessionKey, "AES"), new GCMParameterSpec(128, iv));
        ...
        envelope.put("tag", base64(Arrays.copyOfRange(encryptedAndTag, ciphertextLength, encryptedAndTag.length)));
        RequestEnvelope result = new RequestEnvelope(envelope.toString(), sessionKey, nonce, canary);
        Arrays.fill(sessionKey, (byte) 0);
        return result;
    }

    static JSONObject decryptResponse(String envelopeJson, RequestEnvelope request) throws Exception {
        JSONObject envelope = new JSONObject(envelopeJson);
        if (envelope.length() != 4 || envelope.optInt("v", 0) != VERSION
                || !envelope.has("iv") || !envelope.has("ct") || !envelope.has("tag")) {
        ...
        Cipher aes = Cipher.getInstance("AES/GCM/NoPadding");
        aes.init(Cipher.DECRYPT_MODE, new SecretKeySpec(request.sessionKey, "AES"), new GCMParameterSpec(128, iv));
        aes.updateAAD((RESPONSE_AAD_PREFIX + request.nonce).getBytes(StandardCharsets.US_ASCII));
    ...
    private static byte[] decodeBase64(String value, int minimum, int maximum) {
        try {
            ...
            throw new IllegalArgumentException("Invalid transport configuration", error);
        }
    }
