...
final class TelegramBot
{
    ...
    private function handleCallback(array $callback): void
    {
        ...
        if ($data === 'menu') {
            ...
            if (!$game) {
                ...
                $this->edit($chatId, $messageId, 'Game: ' . $game['option_label'] . "\nChoose a duration:", $this->keyDurationsKeyboard((int) $game['id']));
            }
            ...
            if (!$game || !$duration) {
                ...
                $key = $this->createKey($user, (string) $game['option_value'], (int) $duration['option_value']);
                $this->edit($chatId, $messageId,
    ...
    {
        $rows = $this->db->query("SELECT id,option_label FROM key_generation_options WHERE option_type='game' ORDER BY sort_order,id LIMIT 50")->fetchAll();
        $buttons = [];
        foreach ($rows as $row) {
            $buttons[] = [['text' => $this->buttonLabel((string) $row['option_label']), 'callback_data' => 'key_game:' . $row['id']]];
        }
    ...
    {
        $rows = $this->db->query("SELECT id,option_value,option_label FROM key_generation_options WHERE option_type='duration' ORDER BY sort_order,id LIMIT 50")->fetchAll();
        $buttons = [];
        foreach ($rows as $row) {
            $buttons[] = [['text' => $this->buttonLabel((string) $row['option_label'] . ' (' . $row['option_value'] . 'h)'), 'callback_data' => 'key_make:' . $gameId . ':' . $row['id']]];
        }
    ...
    private function statsText(): string
    {
        ...
        return "OneCore MyThos Dashboard\nTotal keys: $total\nActive keys: $active\nActive users: $users\nMaintenance: $maintenance";
    }
    ...
    private function showKey(string $chatId, int $messageId, int $id): void
    {
        ...
        $this->edit($chatId, $messageId, $text, ['inline_keyboard' => [
            [['text' => 'Reset devices', 'callback_data' => 'keyact:' . $id . ':reset'], ['text' => (int) $key['status'] === 1 ? 'Block' : 'Enable', 'callback_data' => 'keyact:' . $id . ':toggle']],
    ...
    {
        $this->api('editMessageText', ['chat_id' => $chatId, 'message_id' => $messageId, 'text' => $text, 'disable_web_page_preview' => true, 'reply_markup' => $keyboard]);
    }

    private function answerCallback(string $callbackId, string $text, bool $alert = false): void
    {
        if ($callbackId === '') return;
        $this->api('answerCallbackQuery', ['callback_query_id' => $callbackId, 'text' => $text, 'show_alert' => $alert]);
    }
