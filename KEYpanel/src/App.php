...
final class App
{
    ...
    private function setup(): never
    {
        ...
        $body = '<section class="auth"><div class="card"><h1>First-time setup</h1>'
            . '<p class="lead">Install OneCore MyThos KEYpanel and create the first owner.</p>'
            ...
            . '<button type="submit">Install secure panel</button></form></div></section>';
    ...
    private function login(): never
    {
        ...
        $body = '<section class="auth"><div class="card"><h1>Secure access</h1>'
            . '<p class="lead">Sign in to OneCore MyThos license control.</p><form method="post">'
            . Security::csrfField()
            ...
            . '<button type="submit">Authenticate</button></form></div></section>';
    ...
    private function dashboard(): never
    {
        ...
        $body = '<h1>Dashboard</h1><p class="lead">License, user and security activity overview.</p><div class="grid">'
    ...
    private function keys(): never
    {
        ...
        $body = '<h1>Keys</h1><p class="lead">OneCore.Vip validates these <code>keys_code</code> records through encrypted <code>/api/v2/connect</code>.</p>' . $output . $form
    ...
    private function settings(): never
    {
        ...
        if ($this->isPost()) {
            ...
            $modname = substr(trim((string) ($_POST['modname'] ?? 'OneCore MyThos')), 0, 100);
            $credit = substr(trim((string) ($_POST['credit'] ?? 'OneCore MyThos')), 0, 255);
        ...
        $body = '<h1>Server settings</h1><p class="lead">Values returned by the encrypted OneCore.Vip key checker.</p><section class="card">'
            ...
            . $generationOptions
            . '<section class="card"><h2>Encrypted loader API</h2><p>Copy this public key to GitHub Actions Variable '
            . '<code>JAVA_LOADER_API_PUBLIC_KEY_B64</code>. The private key stays protected in <code>runtime/</code>.</p>'
            . '<textarea class="mono" rows="6" readonly>' . h($publicKey) . '</textarea>'
            . '<p>Package binding: <code>' . h(Env::get('EXPECTED_ANDROID_PACKAGE')) . '</code><br>Certificate SHA-256: <code>'
            . h(Env::get('EXPECTED_ANDROID_CERT_SHA256')) . '</code></p></section>'
